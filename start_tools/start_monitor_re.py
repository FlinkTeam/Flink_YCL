...
def get_topics():
	...
	for topic in topics.split('\n'):
		...
		if re.match(normal_label,topic):
...
def start_jobs(topics, running_jobs):
	...
	if normal_label in running_jobs or len(topics[1]) == 0 :
		return
	else:
		start_job(normal_label)
		return
...
def start_job(intopic):
	...
	command="{0}/bin/flink run -d -p {1} {2} --input-topic \"{3}\" --output-topic {4} --bootstrap.servers {5} --zookeeper.connect {6} --group.id {7} --dbhost {8} --dbname {9} --username {10} --password {11} --flink.partition-discovery.interval-millis {12}".format(FLINK_HOME,PARALLELISM ,JAR_LOCATION ,intopic,outputtopic,BOOTSTRAP_SERVERS,ZOOKEEPER,GROUP_ID,DBHOST,DBNAME,DBUSERNAME,DBPASSWORD,INTERVAL)
	print(command)
