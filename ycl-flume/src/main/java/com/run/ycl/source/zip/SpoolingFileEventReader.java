...
public class SpoolingFileEventReader implements ReliableEventReader {

    private static final Logger logger = LoggerFactory
            .getLogger(SpoolingFileEventReader.class);
    ...
    private Map<String, List<String>> itemsMap = new HashMap<String, List<String>>();

    /**
     * Create a ReliableSpoolingFileEventReader to watch the given directory.
     */
    private SpoolingFileEventReader(File spoolDirectory,
        ...
        if (!deletePolicy.equalsIgnoreCase(DeletePolicy.NEVER.name()) &&
                ...
                !deletePolicy.equalsIgnoreCase(DeletePolicy.BACKUP.name())) {
            throw new IllegalArgumentException("Delete policies other than " +
                    "NEVER  IMMEDIATE and BACKUP are not yet supported");
        }
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Initializing {} with directory={}, metaDir={}, " +
                            "deserializer={}",
                    new Object[]{
                            SpoolingFileEventReader.class.getSimpleName(),
        ...
        try {
            ...
            logger.info("Successfully created and deleted canary file: {}", canary);
        } catch (IOException e) {
    ...
    public List<Event> readEvents(int numEvents) throws IOException {
        if (!committed) {
            if (!currentFile.isPresent()) {
                logger.info("File should not roll when " +
                        "commit is outstanding. file:" + dealingFile.getAbsolutePath());
                committed = true;
    ...
    private void fillHeader(List<Event> events) {
        ...
        if (annotateBaseName) {
            basename = currentFile.get().getFile().getName();
            for (Event event : events) {
    ...
    private void retireCurrentFile() throws IOException {
        ...
        }/* else if (deletePolicy.equalsIgnoreCase(DeletePolicy.BACKUP.name())) {
            moveCurrentFile(fileToRoll);
        }*/ else {
     ...
     * @throws IOException
     */
    /*private void moveCurrentFile(File fileToMove) throws IOException {
        logger.info("Preparing to move file {}", fileToMove);
        logger.info("fileToMove's parent directory is : {}", fileToMove.getParent());
        if (!fileToMove.exists()) {
            logger.warn("Unable to move nonexistent file: {}", fileToMove);
            return;
        }
        if(dataSource.equalsIgnoreCase(EnumDataSource.CRAWLER.name())) {
        	if (!FileUtils.moveFile(fileToMove, backupPath + DateUtils.getNowString())) {
        		throw new IOException("Unable to move spool file: " + fileToMove);
        	}
        } else {
        	String parent = fileToMove.getParent();
        	String preDir = parent.substring(parent.lastIndexOf("/"));
        	if (!FileUtils.moveFile(fileToMove, backupPath + DateUtils.getNowString() + preDir)) {
        		throw new IOException("Unable to move spool file: " + fileToMove);
        	}
        }
        // now we no longer need the meta file
...
//              decodeErrorPolicy);
            EventDeserializer deserializer = new BcpZipEventDeserializer(deserializerContext, file, tracker, itemsMap, inputCharset);
            ...
            //FileUtils.moveFile(file, errorFilePath);
            return Optional.absent();
        } catch (IOException e) {
    ...
    public static class Builder {
        ...
        public SpoolingFileEventReader build() throws IOException {
            return new SpoolingFileEventReader(spoolDirectory, completedSuffix,
